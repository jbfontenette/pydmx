"""The X-Touch Mini's behaviour, with nothing said about how it is reached.

Everything here was found against real hardware and is subtle enough that a
second copy would drift the first time either was touched: which layer is
showing has to be inferred, control ids translate both ways, the LED cache
is per layer, a write to a hidden layer is discarded and so must never be
cached, and a lamp the device turns off by itself has to be put back.

xtouch.py adds MIDI to this; virtualxtouch.py adds a UDP socket. Neither
repeats any of it, and the simulator therefore behaves like the device by
construction rather than by resemblance.

Nothing here imports mido, so a show can be built with nothing installed.
...
class XTouchBase:
    ...
    # --- what a concrete surface must provide -----------------------------
    #
    # Three methods, and nothing else here knows whether they reach a device
    # or a socket. Everything above them -- the layer inference, the
    # translation both ways, the per-layer caches, the lamp put back on
    # release -- was found the hard way against real hardware, and is the
    # last thing that should exist twice.

    def _incoming(self):
        """Yield ('note'|'cc', number, value) for whatever has arrived.

        value is the velocity for a note and the value for a CC; zero means
        a release, which is how note_off and a zero-velocity note_on come to
        the same thing without this having to know about either.
        """
    ...
    def poll(self):
        ...
        """
        events = []
        for kind, number, value in self._incoming():
            ...
            if kind == "cc":
                ...
                if fader is not None:
                    ...
                    events.append(("fader", fader, value))
                continue
    ...
    def button(self, control, state=1, force=False, layer=None):
        ...
        self._send_note(to_note(control, target), ON if state else OFF)
    ...
    def ring(self, number, value, force=False, layer=None):
        ...
        if landed:
            ...
            cache.pop(key, None)
        self._send_cc(RING_CC[LAYER_NAMES[target].upper()][number - 1], value)
